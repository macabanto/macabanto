...
public class Sudoku {
    ...
    public static boolean isGridEqual(int[][]grid,int[][]grid2){
        boolean isSame=true;
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
        ...
        while (game) {
            for (int row = 0; row < 9; row++) {
                for (int col = 0; col < 9; col++) {
                    if (grid2[row][col] == 0) {
                        cellGrid[row][col].possibleValues = checkLines(cellGrid[row][col].possibleValues, grid2, row,
                                col);
                        cellGrid[row][col].possibleValues = checkBox(cellGrid[row][col].possibleValues, grid2, row,
                                col);
                        if (cellGrid[row][col].possibleValues.size() == 1) {
                            grid2[row][col] = cellGrid[row][col].possibleValues.get(0);
                        }
                    }
            ...
            loops++;
            if (isGridEqual(grid,grid2)) {
                game = false;
                System.out.println("GRID and GRID2 ARE EQUAL!");
            }
            System.out.println();
            for (int i = 0; i < 9; i++) {
                for (int j = 0; j < 9; j++) {
